...
<body style="margin:0;padding:32px 16px;background:#eef0f5;color:#0a0c15;font-family:Inter,system-ui,sans-serif;-webkit-font-smoothing:antialiased;">
  <div style="max-width:560px;margin:0 auto;background:#ffffff;border-radius:16px;padding:28px;box-shadow:0 12px 36px -18px rgba(8,12,25,0.18);">
    <div style="font-size:11px;letter-spacing:0.22em;text-transform:uppercase;color:#5d5f6e;font-weight:600;">Brief · Weekly digest</div>
    ...
    <p style="margin:24px 0 0;color:#9ea0ad;font-size:11px;text-align:center;">Generated by Brief · Visit your dashboard for live data.</p>
  </div>
</body></html>`;
}
...
async function sendEmail(html: string): Promise<{ ok: boolean; error?: string }> {
  ...
  try {
    const r = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        from, to,
        subject: `Brief · Week of ${new Date().toLocaleDateString("en-US", { month: "long", day: "numeric" })}`,
        html,
      }),
    });
